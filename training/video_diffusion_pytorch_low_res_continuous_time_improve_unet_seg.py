    ...
    def __init__(
        ...
        for img_dir in os.listdir(folder):
            train_files.append({"image": os.path.join(folder, img_dir),
                                ...
                                #                      img_dir)
                                'text': os.path.join("/ocean/projects/asc170022p/lisun/r3/results/text_embedding_192_standard",
                                                     img_dir)
                                })
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                with self.accelerator.accumulate(self.model):
                    ...
                    # lobe = lobe.to(self.accelerator.device).unsqueeze(dim=1)
                    # airway = airway.to(self.accelerator.device).unsqueeze(dim=1)
                    # vessel = vessel.to(self.accelerator.device).unsqueeze(dim=1)
                    # img = torch.cat([img, lobe, airway, vessel], dim=1)
                    # img = F.interpolate(img, scale_factor=0.25, mode='nearest')
